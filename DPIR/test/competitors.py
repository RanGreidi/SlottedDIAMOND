...
class RandomBaseline:
    ...
    def random_episode(self, env):
        ...
        return paths, np.sum(rewards), delay_data, rates_data, actions
    ...
    def run(self, env, seed=None):
        ...
        for i in range(self.num_trials):
            paths, score, delay_data, rates_data, actions = self.random_episode(env)
            if score > best_score:
                best_score = score
                best_paths = paths
                ...
                choosen_actions = [action[1] for action in actions]
        return best_paths, best_score, save_delay, save_rates, choosen_actions
...
class OSPF:
    ...
    def run(self, env, seed=None):
        ...
        for step in range(env.num_flows):
            ...
            rewards.append(r)
        delay_data = env.get_delay_data(action_as_idx=False)
        rates_data = env.get_rates_data()
...
class DQN_GNN:
    ...
    def run(self, env, seed=None):
        ...
        actions.sort(key=lambda x: x[0])
        choosen_actions = [action[1] for action in actions]
        return paths, actions, delay_data, rates_data, choosen_actions
    ...
    def _select_action(self, state, env):
        ...
        for flow_idx in free_flows_idx:
            ...
            for path in env.possible_actions[flow_idx]:
                ...
                delay_data = test_env.get_delay_data(action_as_idx=False)
