...
class TestvsCompetitors:
    ...
    def __call__(self, data_paths_list=None, **kwargs):
        ...
        for episode in range(self.num_episodes):
            ...
            if save:
                ...
                # generate env
                Gloval_env, env_configurations = generate_env(**env_args)
                ...
                for slot in range(self.num_slots):
                    ...
                    # initialize data for first slot
                    ...
                    if Algos_step_envs['SlotedDIAMOND'].flows:
                        ...
                        all_slotted_paths[slot].append(slotted_diamond_paths)
                    ...
                    if Algos_step_envs['DIAMOND'].flows:
                        diamond_rates_data, diamond_delay_data = run_predefined_actions(Algos_step_envs['DIAMOND'],
                                                                                        self.first_step_actions['DIAMOND'])
                ...
                # Modify Global_data to be averaged over all episodes
        ...
        # save_fig = False
        # subfolder_path = plot_algorithm_metrics(Episode_Avarge_data, num_flows=num_flows, seed=seed, Gloval_env=Gloval_env,
        #                        graph_mode=kwargs['graph_mode'], save_fig=save_fig)
    ...
    def create_slot_flows(self, Algos_Global_flows):
        ...
        output: flows list for each algo for a single slot
        ...
        for algo, flows in Algos_Global_flows.items():
            new_Algos_Global_flows[algo] = self.generate_flows_with_fixed_pkt(flows,
...
if __name__ == "__main__":
    ...
    slot_duration = 1
    num_slots = 500  # 200, 700

    for GRAPH_MODE in ['random']:  # , 'geant', 'nsfnet'
        for trx_power_mode in ['equal', 'rayleigh', 'steps']:  # , 'rayleigh', 'steps'
            ...
            for num_flows_idx, num_flows in enumerate(flows):
                ...
                data, labels, average_rates_through_time, average_delays_through_time, subfolder_path = alg(data_paths_list=None,
