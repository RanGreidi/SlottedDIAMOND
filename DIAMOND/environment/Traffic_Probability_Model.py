...
class Traffic_Probability_Model:
    def __init__(self,
        ...
        if initial_state not in self.states:
            raise ValueError(f"Initial state {initial_state} is not in the list of possible states.")
    ...
    def step(self):
        ...
        next_state_index = self.rng.choice(len(self.states), p=self.transition_matrix[current_state_index])
...
if __name__ == "__main__":
    ...
    mc = Traffic_Probability_Model( source=0,
                                    destination=1,
                                    initial_state=1005,
                                    flow_name=0,
